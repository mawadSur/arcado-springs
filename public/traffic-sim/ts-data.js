  ...
  TS.DATA = {
    ...
    hotspots: [
      { id: "hs-mainX", label: "Main intersection: Arcado Rd & Killian Hill Rd", type: "intersection", ll: [33.883026, -84.11597],
        beforeText: "This corner is where everything piles up. With no separate left-turn lane, one left-turning car holds up everyone behind it, so the signal's green can't clear the approach — and at rush hour more cars arrive each cycle than it releases, stretching the line well back down Arcado Road.",
        afterText: "With a dedicated turn lane and an organized site entrance, cars turning into Arcado Springs get out of the way of through traffic, and fewer cars reach the corner in the first place. Now each green clears the line instead of leaving cars for the next cycle, so the queue at the signal is noticeably shorter and the corner moves more cars each hour." },
      ...
      { id: "hs-siteAccess", label: "Site access management", type: "site-access", ll: [33.880133, -84.119126],
        ...
        afterText: "Access management — consolidating entries, channelizing turns, and capturing trips on-site — lets turning traffic use predictable gaps and keeps the through traffic flowing, so the signal at Killian Hill clears each approach more fully." },
    ...
    presentationSteps: [
      ...
      { id: "step-bottlenecks", order: 2, title: "Where it breaks down", scenario: { id: "am-peak", side: "before" },
        ...
        narration: "Look at the corner. With no separate turn lane, one left-turning car stalls the lane behind it, and at rush hour more cars arrive each cycle than the signal's green can release — so the queue grows cycle after cycle and spills back down Arcado Road. This is the bottleneck.", focusHotspot: "hs-mainX" },
      { id: "step-proposed", order: 3, title: "The proposed fixes", scenario: { id: "am-peak", side: "after" },
        ...
        narration: "Now the build-out: a single consolidated driveway, managed site access, a dedicated left-turn lane, and protected pedestrian crossings to the shops and green — easing the load on the Killian Hill signal.", focusHotspot: "hs-driveway" },
